...
export default function SelectBase({
    ...
    const formClassName = variant === 'primary'
        ? 'border border-lightGrey rounded-sm px-3 py-[7px] w-full overflow-x-auto'
        : 'border-b border-lightGrey px-3 py-[7px] w-full overflow-x-auto';
    ...
    return (
        <div className={`dark:text-white font-light relative ${className}`} tabIndex={0} onBlur={() => setTimeout(() => setOnDrop(false), 150)}>
            ...
            <div
                ref={contentRef}
                className={`
                    absolute mt-1 z-[10] px-5 py-3 text-xl w-full max-h-[40vh] bg-white dark:bg-secondaryBlack border-[2px] border-lightGrey shadow overflow-hidden overflow-y-auto rounded-lg
