...
const purchaseOrderDataStateValue = createSlice({
    ...
    reducers: {
        ...
        deleteProductData: (state: PurchaseOrderData, action: PayloadAction<{ productType?: 'normal' | 'special', index: number }>) => {
            ...
            if (productType === 'special' && state.specialProductDatas.length >= 0) {
                state.specialProductDatas = deleteAndReassignId(state.specialProductDatas, index);
            }
            else if (state.productDatas.length > 1) {
                state.productDatas = deleteAndReassignId(state.productDatas, index);
            }
